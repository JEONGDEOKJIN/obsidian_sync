...
⭐ '비동기 처리 결과' 를 가지고 또 다시 '비동기 함수를 호출! -> 함수 호출이 중첩 -> 복잡도 증가!'
*/
...
// 1. 콜백함수 내부에서 try-catch 문 실행 
	...
	// 이게 작동하는 이유는, throw new Error('Error!'); 이 콜백함수가 작동하는 코드 블록에, try-catch 문이 있기 때문 ⭐⭐⭐⭐⭐ 
...
- promise 기본 기능
	...
	- ⭐⭐⭐ promise 는 비동기 처리의 '상태' 와 '처리 결과' 를 관리하는 객체 (출처 : 모던 자바스크립트 p852)


- 프로미스는 어떻게 비동기 처리의 결과값을 컨트롤 하는가 ⭐⭐⭐⭐⭐  
	- [전제조건] 
		...
		- resolve 가 실행이 되는 경우의 흐름
			...
			- 후속처리 [처리결과를 가지고, 무엇을, 어떻게 할 것 인가.]
				...
				- 'then 메서드' 에서는, 전달받은 인자로, 뭔가를 한다. 
					- then 메서드의 첫 번째 인자는, 성공한 경우 호출되고, 
					- then 메서드의 두 번째 인자는, 실패한 경우 호출
				- 결과값 : '프로미스' 반환 / 이게 의미있는 이유는, 2번째 비동기 처리의 매개변수가 될 수 있어서 ❓❓❓ (출처 : 모던 자바스크립트 p854)
				
			- 공통처리
				- finally 메서드 : 무조건 한번 실행 (매개변수 1개) / 프로미스 반환

		- reject 가 실행되는 경우의 흐름
