...
- 그러면, console.log 끝나고 처리되는 값에 어떻게 접근? 
	...
	- '비동기 함수 내부' 로 접근할 수 밖에 없음 -> 따라서, 비동기 함수의 매개변수로, '원하는 기능을 갖고 있는 함수' 를 전달 (이렇게 전달되는 함수를 콜백함수, 라고 함.)
...
- 'onload 이벤트가 발생했을 때 실행되는 콜백함수(xhr.onload 이벤트 핸들러의 프로퍼티에 바인딩된 이벤트 핸들러)' 는 언제 실행되나? 원하는 곳에서 콜백함수의 처리값을 콘솔로 보려면? 여기에서 왜 undefine 가 찍히는가. ⭐⭐⭐⭐⭐⭐ 
``` js 
...
### 콜백 헬(callback hell) 이 등장하는 경우 : 연쇄적 요청(Chained Requests)
``` js
/* EX) 
...
- 그러면, 콜백패턴에서 error 를 캐치 하려면? (📛📛📛 이 코드 패턴 계속 틀림)  
``` js  
