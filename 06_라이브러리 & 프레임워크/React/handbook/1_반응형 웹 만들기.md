...
1. '컬럼의 개수를 고정' 시킬 것 인가, 아니면, '브라우저 너비에 최대한 채울 것 인가(가변적).' 선택하기 
	- 컬럼 개수 고정 : grid-template-rows: repeat(3, minmax(120px, auto))
	- 너비를 최대한 컬럼으로 채운다 
...
2. - 최대한 채운 후, 비어있는 공간을 어떻게 채울 것 인가. 
	 - 가만히 둔다 
	 - 채운다 : grid-auto-flow

3. grid container 하위 구성 요소 영역 지정
	- 숫자 방식 : 그리드 디자인 시스템을 그대로 반영할 수 있다는 점에서, 우선, 이걸로 시도
	- 영역 이름 지정 방식
	- 진행 과정(process)
		1) grid 컨테이너 하위 컴포넌트(마크업) 구조 만들기 
		2) breakpoint 별로 지정하기
	- 주의할 것 
...
5. 불변 gutter 어떻게 설정 할 것 인가.
	- grid container 중 gap 속성에 설정 
		  .card-list {
		    display : grid;
...
3. item 의 비율을 어떻게 고정 시킬 것 인가. 
	...
	2. '브라우저 너비 변화(600px ~ 1200px 사이 일 것)'에 따라서, 'column 너비' 를 '가변적' vs '고정적' 으로 할 것 인가. 
		- 고정적이라면, 
