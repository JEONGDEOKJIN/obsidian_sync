...
# 1️⃣ 최종 CSS 작업에서 배우는 점 


```
1. 기능이 완성된 상태에서 -> '목차' 뼈대를 갖고 오고 

2. 클래스 명이 아니라, '목차' 를 따온다 ⭐⭐⭐ 
...
1. CREATE 페이지에, 댓글 작성 완료 버튼에, `작성 완료` 버튼 놓기

![](https://i.imgur.com/277njBK.png)


2. EDIT 에서 태그 가져오고, 수정 까지 하기 
...
```
- css 부분에서 바뀐 건 없음. 
	- https://www.diffchecker.com/tXajzBQq/

- js 부분에서 바뀐 건 없음. 

- js 부분에서 작동해야 하는 변수들이, 배열들, 함수들이 제대로 붙어 있는지 보자. 
...
1. form.append("content" , content.value);
	id = content 인것에 담긴 value 를 가져오는건데, 
	`textarea` 태그를 써서 문제가 될 수도 ⭐⭐⭐ 
...
```
- css 변경 사항 
https://www.diffchecker.com/7QZ8quKO/

- html 변경 사항 
https://www.diffchecker.com/b5azersQ/

- js 는 변경된 건 없음. 
	- 다만, class 변경, id 변경, 추가된, 위계 질서 변경 들의 영향 요인 파악 필요 
	- 사용자 흐름 따라가보기 

✅ 여기도 textarea 태그가 문제일 수도 있음. 
```
...
```
  original: Error: Access denied for user ''@'localhost' (using password: YES)
      at Packet.asError (C:\Users\user11\Desktop\kga\projects\monami_local - 복사본 (4)\backEnd\node_modules\mysql2\lib\packets\packet.js:728:17)
      at ClientHandshake.execute (C:\Users\user11\Desktop\kga\projects\monami_local - 복사본 (4)\backEnd\node_modules\mysql2\lib\commands\command.js:29:26)
      at Connection.handlePacket (C:\Users\user11\Desktop\kga\projects\monami_local - 복사본 (4)\backEnd\node_modules\mysql2\lib\connection.js:490:32)
      at PacketParser.onPacket (C:\Users\user11\Desktop\kga\projects\monami_local - 복사본 (4)\backEnd\node_modules\mysql2\lib\connection.js:95:12)
      at PacketParser.executeStart (C:\Users\user11\Desktop\kga\projects\monami_local - 복사본 (4)\backEnd\node_modules\mysql2\lib\packet_parser.js:75:16)
      at Socket.<anonymous> (C:\Users\user11\Desktop\kga\projects\monami_local - 복사본 (4)\backEnd\node_modules\mysql2\lib\connection.js:102:25)
...
- 해결은 이렇게 `.env` 파일을 작성 
```
ACCESS_TOKEN_KEY = monamiAccessKey
REFRESH_TOKEN_KEY = monamiRefreshKey

DATABASE_PASSWORD = mysqlpwdj
DATABASE_USERNAME = root
DATABASE_NAME = monami
...
- 그러면, 되었던 코드랑, 안 되는 코드랑 비교해보자. 


- 음... js 는 크게 문제 없었던 것 같음. 
https://www.diffchecker.com/xxegLS03/

- 이건 HTML 인데, 음... 문제 발견은 못 함
https://www.diffchecker.com/igkMdN5x/

- css 에서는 다르네 
https://www.diffchecker.com/KFd0jWSb/

```

```
좀 더 보려면, 내 눈 앞에, 이제 그 게시물들이 보여야 해 

사진을 못 가져오는 거면, db 에서 연결해서 테스트 해주면 되지 않을까. 
```
...
```
axios 자체가 연결이 안 되는 문제 
이건, 설치가 제대로 안 되어 있을 때 나옴 
그러면 위로 script 로 가봐야 하는데, 
그러면, 이렇게 중복되어진게 보임. 

```

![](https://i.imgur.com/eB4GtA0.png)
...
git pull request 만 조심하고,

내가 브랜치니까, 내가 그냥 마음대로 써도 된다. 
