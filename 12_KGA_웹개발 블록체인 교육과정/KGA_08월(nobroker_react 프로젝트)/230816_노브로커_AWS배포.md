...
- [vs code 작성하면서 확인할 것]
	- 프록시 경로 (참고  : [[230816_노브로커_AWS배포#^09ba52]]) #📛📛📛_보완필요_잘모르겠음 
...
#### 명령어 요약 
- 출처 : [[230816_노브로커_AWS배포#EC2 인스턴스 만들고 연결하기]]
``` bash
1. aws EC2 인스턴스 들어가기
...
#### ubuntu 디렉토리에서 git 설치 (출처 : [[230816_노브로커_AWS배포#ubuntu 디렉토리에가서 -> git 설치]])
``` bash
[ubuntu 디렉토리로 이동]
cd /home/ubuntu
...
#### 탄력적 ip 추가하기 
- 출처 : [[230816_노브로커_AWS배포#탄력적 ip 추가 하기]]



#### Route53 에서 퍼블릭 ipv4 값 넣기 
 - 출처 : [[230816_노브로커_AWS배포#Route 53 에서 퍼블릭 ipv4 값을 넣어주기]]



#### 가상 인스턴스 안에 SQL 연결 | DB 랑 연결 
- 출처 : [[230816_노브로커_AWS배포#가상 인스턴스 안에 SQL 연결하기 DB 랑 연결해주기]]
...
##### 2. workbench 에서 연결 
- 출처 : [[230816_노브로커_AWS배포#2. Workbench 에서 연결]]
``` bash 
1. aws 에서 퍼블릭 IPV4 DNS 가져오기 
	- 탄력적 ip 주소 아님! 
	- 퍼블릭 IPv4 DNS 임
...
##### 3. not fetched 문제 해결 📛📛📛📛📛📛 
- 출처 : [[230816_노브로커_AWS배포#3. not fetched 문제 해결]]
``` bash
# 230824 아직 안 해봤음 📛
...
- aws 인스턴스 접속해서, nginx 설치 및 설정 
	- 출처 : [[230816_노브로커_AWS배포#aws 인스턴스 접속해서, ngix 설치 및 설정 ⭐⭐⭐ 이 부분에서 오류가 많이 남]]
``` bash 
# 버전 업그레이드 
...
- 프록시 설정 | 출처 : [[230816_노브로커_AWS배포#교수님이 해주신 부분]]
``` js
= , ~ 을 설정하여, api 하위 요소가 붙어서, 요청이 들어갈 수 있는 가능성을 알게 됨  
```
![](https://i.imgur.com/nW6k81z.png)
...
- 출처 : [[230816_노브로커_AWS배포#'SSH 방식' 으로 환경 변수 설정하기]]
...
- 출처 : [[230816_노브로커_AWS배포#프론트를 빌드하고 -> 빌드 파일을 인스턴스의 루트 디렉토리로 가져와서 -> 리액트앱을 실행하기 ⭐⭐⭐]]
